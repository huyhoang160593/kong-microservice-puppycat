...
const root: FastifyPluginAsync = async (fastify, _opts): Promise<void> => {
		...
		async (request, response) => {
			...
			if (!foundUser) {
				throw fastify.httpErrors.badRequest(
					`We can find the user with the id ${id} in the database`,
				);
			}
	...
	fastify.withTypeProvider<ZodTypeProvider>().post(
		...
		async (request, response) => {
			const {email, avatarUrl, name, password} = request.body;
	...
	fastify.withTypeProvider<ZodTypeProvider>().post(
		...
		async (request, response) => {
			...
			const userFound = await fastify.prisma.user.findFirst({
				where: {email},
			});
			const isPasswordCorrect = userFound
				? await fastify.bcrypt.compare(password, userFound.passwordHash)
				: false;
